-- PTT Nexus Online — organisation tags and submissions
--
-- Everything needed for the Tags and Submissions screens, and for Nexus to
-- push unmatched schools. Run it whole, against the same database the app
-- reads. It touches nothing that is already there: no table is altered, no
-- row is rewritten, and no column is dropped.
--
-- Safe to run more than once. Every table is IF NOT EXISTS and every insert
-- is guarded, so a second run adds whatever a first could not and changes
-- nothing else.
--
-- Two parts:
--   1. organization_submissions — schools a meet met that this database does
--      not have. Nothing here reaches organizations until somebody approves
--      it on the Submissions screen.
--   2. org_tags / organization_tags — how a school is classified, as things
--      a meet can filter by. The tags at the end are derived from the levels,
--      divisions, conferences and state associations the organisations
--      already carry, so nothing has to be re-entered.
--
-- The one thing it cannot fill in is which class a school is in. No column
-- has ever held that, which is why the classes exist; they go on from the
-- Tags screen.
